...
def main(wf):
    if len(wf.args):
        ...
        if len(query) > 2:
            url = 'https://api.redfoxsanakirja.fi/redfox-api/api/basic/autocomplete/eng/{}'.format(query)
            log.debug(url)
            ...
            for w in result:
                w1 = make_url(w)
                log.debug(w)
                new_url = "https://api.redfoxsanakirja.fi/redfox-api/api/basic/query/eng/fin/{}".format(w1)
